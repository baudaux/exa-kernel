...
int exafs_record_store(struct exafs_ctx * ctx, enum meta_op op, void * buffer, int len) {
  ...
  struct meta_record * record = malloc(l); // includes last crc
  ...
  uint32_t * ptr = (data+(l-sizeof(uint32_t)));

  *ptr = crc;

  int res = ctx->write(ctx, ctx->meta_log_head, record, l);

  emscripten_log(EM_LOG_CONSOLE, "exafs: write record size=%d  -> res=%d", l, res);
